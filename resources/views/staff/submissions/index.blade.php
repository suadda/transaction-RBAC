<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            ...
            <a href="{{ route('staff.dashboard') }}" class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
                + Buat Pengajuan
            </a>
        </div>
    </x-slot>
                    ...
                    <tbody>
                        @forelse ($submissions as $s)
                            <tr class="border-b">
                                ...
                                <td class="px-4 py-3">{{ $s->date->format('d/m/Y') }}</td>
                                <td class="px-4 py-3">{{ $s->category->name }}</td>
                                <td class="px-4 py-3">Rp {{ number_format($s->amount, 0, ',', '.') }}</td>
                                <td class="px-4 py-3">@include('partials.status-badge', ['status' => $s->status])</td>
                                <td class="px-4 py-3">
                                    <a href="{{ route('staff.submissions.show', $s) }}" class="text-blue-600 hover:underline">Detail</a>
                                </td>
                            </tr>
                        @empty
